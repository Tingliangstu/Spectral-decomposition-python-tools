    ...
    **** For different direction spectral SHC **** (Added by liang ting)
    ...
    def _calcFC(self, fileprefix, LAMMPSInFile):
        ...
        with fcCalc(fileprefix) as fc:
            ...
            fc.writeToFile()
            print('\nForce constant matrix file generate done')
    ...
    def postProcess(self):
        ...
        if (vel_sample_steps * self.dt_md) != (self.sampleTimestep):    # Error checking
        	 sys.exit('LAMMPS ERROR: Different dump stride given in '
        ...
        print('The number of sampling frequencies is ' + str(Nfreqs) + '\n')        
...
if __name__ == "__main__":
    ...
    Tem_jump = 60                                                ## 60K is the temperature jump
    ...
    plt.plot(x_Frequency, accumulated_ITC, 'or', label="Accumulated thermal conductance") 
    plt.xlabel('w/(2*pi) (THz)')
    ...
    plt.legend(fontsize=15, loc='best')
    plt.show()
    plt.savefig(fileprefix+'_accumulated_ITC.eps')